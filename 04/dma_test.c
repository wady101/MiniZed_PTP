...
int getNumber (){
  ...
  //--------------------------Talks about the how the user input is parsed
	while(1){
		byte = 0x00;
		digit = 0;
		digitIndex = 0;
...
int SetupIntrSystem(XScuGic *GicPtr, XDmaPs *DmaPtr)
{
  ...
  //extern void Xil_ExceptionRegisterHandler(u32 Exception_id, 				
          ...
          //void *Data);----------------------Just a smart handler which calls to a ISR based on the Data ptr
...
int main (void) {
	...
	Status = XDmaPs_CfgInitialize(&Dma,DmaCfg,DmaCfg->BaseAddress); //-------DMA Initialized in PS
